...
public class DeleteServiceActivity extends AppCompatActivity {
    ...
    public void deleteAction(Integer _materialID, DBHelperMaterial _dbMaterial) {
        ...
        formHelper.setInfoText("Es wurde die Wartung: " + m.get_name() + " " + m.get_type() + " " + m.get_year() + " gelöscht.", infoText);
    }
    ...
    public void updateAction(Integer _materialID, DBHelperMaterial _dbMaterial) {
        ...
        if (!"".equals(sellDate.getText().toString()) && !"".equals(sellPrice.getText().toString()))
        {
            ...
            formHelper.setInfoText("Es wurde die Wartung: "+m.get_name()+" "+m.get_type()+" "+m.get_year()+" verkauft.",infoText);
        }
        else{
            formHelper.setInfoText("Um eine Wartung als Verkauft zu makieren, müssen die Felder Verkaufspreis und - Datum ausgefüllt werden.",infoText);
        }
    }
    ...
    public void noMaterialMessage(){
        formHelper.setInfoText("Es ist keine Wartung in der Auswahl verfügbar", infoText);
    }
